...
static void ata_write_sector(uint32_t lba, const uint8_t* buffer) {
    ata_wait_bsy();
    outb(0x1F6, 0xE0 | ((lba >> 24) & 0x0F));
    outb(0x1F2, 1);
    outb(0x1F3, (uint8_t)lba);
    ...
    ata_wait_drq();
    for (int i = 0; i < 256; i++) {
        uint16_t w = buffer[i*2] | ((uint16_t)buffer[i*2 + 1] << 8);
        outw(0x1F0, w);
    }
    ata_wait_bsy();
...
static void load_filesystem() {
    ...
    for (size_t i = file_count; i < 16; i++) {
        files[i].name[0] = 0;
        files[i].data[0] = 0;
    }
}
...
void shell() {
    ...
    while (1) {
        ...
        if (!strcmp(cmd, "help")) {
            print("Commands: help ls cat echo clear halt reboot mkdir nano sync\n");
